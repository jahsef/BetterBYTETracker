"""Compare cuda-bytetracker output against ultralytics BYTETracker on identical inputs.

Requires ultralytics to be installed (test-only dependency).
Run: python -m pytest cuda-bytetracker/tests/test_parity.py -v
"""
...
# ---------------------------------------------------------------------------
# Fake "results" object that both trackers accept
# ---------------------------------------------------------------------------
...
def _make_detections(seed: int = 42) -> list[FakeResults]:
    ...
    # Base positions: 3 objects at different locations
    base_xywh = np.array(
        [[100, 100, 40, 60],
         [250, 200, 50, 80],
         [400, 150, 30, 50]],
    ...
    for f in range(n_frames):
        # Add small per-frame displacement
        ...
        # Shift base slightly to simulate constant velocity
        base_xywh[:, 0] += 3
        base_xywh[:, 1] += 2
    return frames
...
def _run_ultra(frames: list[FakeResults]) -> list[np.ndarray]:
